...
```
minikube kubectl apply -f postiz-deployment.yaml
```

If it does not work for microk8s then try:

```
microk8s kubectl apply -f postiz-deployment.yaml
```

You should see in the `default` namespace defined for kubernetes all the new objects.
The corresponding service is called `postiz`.
...
To find the mapped ports for port 80 just check the "Internal Endpoints" of the service "postiz"
